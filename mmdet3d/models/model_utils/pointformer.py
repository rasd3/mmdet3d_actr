...
class BasicDownBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.local_chunk = LocalTransformer(npoint, radius, nsample,
                                            dim_feature, dim_hid, nhead,
                                            num_layers, norm_cfg, enc_ratio,
                                            local_drop, prenorm)
        self.global_chunk = GlobalTransformer(dim_hid, dim_out, nhead,
                                              num_layers, norm_cfg, enc_ratio,
                                              npoint, global_drop, prenorm)
        if use_decoder:
            self.combine_chunk = LocalGlobalTransformer(
                dim_hid, dim_hid, nhead, 1, norm_cfg, ratio, mem_pts, npoint,
                decoder_drop, dim_feature, prenorm)
    ...
    def forward(self, xyz, features):
        ...
        if self.use_decoder:
            combined = self.combine_chunk(new_xyz, xyz, local_features,
                                          features)
...
class Pointformer(nn.Module):
    ...
    def __init__(self,
        ...
        for i, (np, r, ns, ratio, use) in enumerate(
                zip(num_points, radius, num_samples, ratios, use_decoder)):
            if i < self.num_sa - 1:
                self.SA_modules.append(
                    BasicDownBlock(np, r, ns, bc, bc, bc * 2, num_heads,
                                   num_layers, norm_cfg, ratio, mem_pts,
                                   use_lin_enc, use, local_drop, global_drop,
                                   decoder_drop, prenorm))
                self.nc_outs.append(bc * 2)
            else:
                self.SA_modules.append(
                    BasicDownBlock(np, r, ns, bc, bc, bc, num_heads,
                                   num_layers, norm_cfg, ratio, mem_pts,
                                   use_lin_enc, use, local_drop, global_drop,
                                   decoder_drop, prenorm))
        ...
        self.FP_modules = nn.ModuleList([
            PointFPModule([
                self.nc_outs[-1] + self.nc_outs[-2], fp_channels[0][0],
                fp_channels[0][1]
            ],
                          norm_cfg=norm_cfg),
            PointFPModule([
                self.nc_outs[-3] + fp_channels[0][1], fp_channels[1][0],
                fp_channels[1][1]
            ],
                          norm_cfg=norm_cfg)
        ])
    ...
    def forward(self, points):
        ...
        features = self.feature_conv(points[..., 3:].transpose(
            1, 2).unsqueeze(-1)).squeeze(-1)
