...
class ParallelMVXMono3D(DynamicMVXFasterRCNN):
    ...
    def input_visualize(self, imgs, gt_bboxes):
        img_norm_cfg = dict(mean=np.array([103.530, 116.280, 123.675]),
                            std=np.array([1.0, 1.0, 1.0]),
                            to_rgb=False)
        global IDX
        mean, std, to_rgb = img_norm_cfg['mean'], img_norm_cfg['std'], img_norm_cfg['to_rgb']
        for idx, (img, bboxes) in enumerate(zip(imgs, gt_bboxes)):
            ...
            for bbox in bboxes:
                img = cv2.rectangle(img.copy(), (bbox[0], bbox[1]), (bbox[2], bbox[3]), (0, 0, 255), 3)
    ...
    def forward_train(self,
        ...
        losses_img = self.img_bbox_head.forward_train(
            img_feats, img_metas, gt_bboxes_cam, gt_labels, gt_bboxes_3d_cam,
